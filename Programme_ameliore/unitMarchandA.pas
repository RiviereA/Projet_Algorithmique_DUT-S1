...
uses PersonnageUnitA,ActionMineAmelioree,gestionEcran,actionForetA;
...
while not sortie do
    ...
    effacerEcran;
		...
		case choix of
			...
			1: begin
				if personnage.epee=1 then											// on utilise des case afin de correspondre au choix du joueur
					begin
						writeln('Vous vendez votre épée en bois, vous gagnez 5Po'); // on affiche ce que l'on fait
						personnage.orPerso:=personnage.orPerso+5;					// on ajoute de l'or au joueur
						personnage.epee := 0;										// on lui retire son epee
					end		

					else	
						writeln('Vous ne disposez pas d''une épée en bois'); // on l'informe, dans le cas ou il n'a pas d'épée en bois de son erreur

				end;
										// et on recommence pour tout les types d'épée
			2 :
        begin
          ...
          begin
			...
			4 :
        begin
					sortie:=TRUE; // on sort de cette section
...
// ON REPETE CE GENRE D ACTION POUR CHAQUE VENTE, ET ON FERA L INVERSE EN CAS D ACHAT
...
begin
  ...
  marchandOrNotMarchand:=random(3)+1;
  if marchandOrNotMarchand=2 then
  begin
			while sortie do
				begin
          ...
          writeln('3 : chasser');
					writeln('4 : sortir de la forêt');
					readln(choix);
          effacerEcran;
					case choix of
						1:marchand(personnage);
						2:recolterBois(personnage);
            3:chasser(personnage);
						4:sortie:=FALSE
						else
              writeln('Vous avez entré une valeur interdite, pour la peine vous quittez la foret et retournez chez vous')
          end;
        end;
  end
    ...
    begin
      writeln('Il n''y a pas de marchand en vue, dommage, mais vous pouvez toujours chasser ou couper du bois !!');
					writeln('1 : récolter du bois');
          writeln('2 : chasser');
					writeln('3 : sortir de la forêt');
          readln(choix);
          effacerEcran;
          case choix of
						1:recolterBois(personnage);
            2:chasser(personnage);
						3:sortie:=FALSE
          end;
     end;
end;
